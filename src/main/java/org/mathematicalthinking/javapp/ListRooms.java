...
public class ListRooms {
  ...
  public static void main(String[] args) {
    try {
      FileInputStream contentStream = new FileInputStream("target/Topic1-cereal_group.jno");
      ObjectInputStream ois = new ObjectInputStream(contentStream);
      String channelID = (String) ois.readObject();
      String applicationClassName = (String) ois.readObject();
      System.out.println(channelID);
      System.out.println(applicationClassName);
  ...
  // System.out.println("SOMETHING NEWER!");
  // // Get a database 'Connection' object
  // ccConn =
  // DriverManager.getConnection("jdbc:mysql://localhost:3306/concertchat?serverTimezone=UTC",
  // "root", "");
  // vmtConn =
  // DriverManager.getConnection("jdbc:mysql://localhost:3306/vmt?serverTimezone=UTC",
  // "root", "");
  // System.out.println("Got connections");
  // Statement stmtCh = ccConn.createStatement();
  // System.out.println("Got stmtCh");
  // // String strSelectCh = "select * from r4349443A31333438303433393839323931
  // order
  // // by PRIMKEY";
  // String strSelectCh = "select * from r4349443A31353236393535323430383835 order
  // by PRIMKEY";
  // // String strSelectCh = "select * from r4349443A31353336323831393331343838
  // order
  // // by PRIMKEY";
  ...
  // int rowCountCh = 0;
  // while (rsetCh.next()) { // Move the cursor to the next row, return false if
  // no more row
  // try {
  // // Integer timeInt = rsetCh.getInt("TIME");
  // // String timeStamp = (new SimpleDateFormat("mm:ss:SSS")).format(new
  // // Date(timeInt));
  ...
  // ObjectInputStream(descCh.getBinaryStream());
  // // channelMessageContainer containes the message in
  // MessageContainer channelMessageContainer = (MessageContainer)
  // objectInputStream.readObject();
  // // MessageContainer channelMessage = (ChannelMessage)
  // // objectInputStream.readObject();
  // objectInputStream.close();

  // objectInputStream = new ObjectInputStream(descCh.getBinaryStream());
  // Object o = objectInputStream.readObject();
  // System.out.println(" desCh message class name is: " +
  // o.getClass().getName());
  // objectInputStream.close();
  ...
  // String strSelect = "select * from aroom r " + "left join community c on
  // r.CommunityID = c.CommunityID "
  // + "left join roomtype rt on r.RoomTypeID = rt.RoomTypeID "
  // + "left join atopic t on r.TopicID = t.TopicID " + "left join subject s on
  ...
  // channelMessageContainer.getGlobalIndex());
  ...
  // TabbedGeogebraBoardDescriptor tgbd = (TabbedGeogebraBoardDescriptor)
  // channelMessageContainer.getMessage();
  // System.out.println(" Descriptor Name: " + tgbd.getName());
  // System.out.println(" Descriptor ID: " + tgbd.getID());
  // Field initFile =
  // TabbedGeogebraBoardDescriptor.class.getDeclaredField("initFile");
  ...
  // // System.out.println (" msgContainer.getMessageID: " +
  // // msgContainer.getMessageID());
  ...
  // // channelMessageContainer.getPersistent());
  // break;
  // default:
  ...
  // String topicName = rset.getString("t.TopicName"); // corresponds with
  // // vmt.mathforum.org/vmtwiki/index.php/VMTTopics/<TopicName>
